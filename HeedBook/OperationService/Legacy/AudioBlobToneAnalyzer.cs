...
namespace OperationService.Legacy {
    public static class AudioBlobToneAnalyzer
    {
        [FunctionName("Audio_Blob_ToneAnalyzer")]
        public static void Run(string msg,
            ExecutionContext dir,
            ILogger log)
        {
            log.LogInformation($"Function started {msg}");
            ...
            try
            {
                log.LogInformation($"Processing audio file tone:{name}");
                ...
                log.LogInformation($"{localAudioFn}");
                try
                {
                    var splitSeconds = 15;
                    ...
                    {
                        log.LogInformation($"{JsonConvert.SerializeObject(curMetadata)}, index {i}");
                        ...
                        try
                        {
                            var res = RecognizeTone(dir, curFn, log);
                        ...
                        {
                            log.LogInformation($"Tone analyzer faied {e}");
                    ...
                    log.LogInformation($"{toneDocs.Count}");
                    ...
                    log.LogInformation("Added document to mongodb database: {doc}", doc.ToJson());
        ...
        public static Dictionary<string, double> RecognizeTone(ExecutionContext dir, string fn, ILogger log2)
        {
            /***********
            ...
            **********
            ...
            var text = cmd.runCMD(Path.Combine(Misc.BinPath(dir), "OpenVokaWavMean-3-0-win64.exe"), fn);
            log2.LogInformation($"{text}");
            try
            {
                var pattern = @"\n\s?(\w+)\s+([\d\.]+)";
