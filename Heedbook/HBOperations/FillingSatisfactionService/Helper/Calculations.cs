...
namespace FillingSatisfactionService.Helper
{
    public class Calculations
    {
        ...
        public List<DialogueClientSatisfaction> TotalScoreCalculate(List<Dialogue> dialogues)
        {
            ...
            var visuals = _context.DialogueVisuals.Where(p => dialoguesIdName.Contains(p.DialogueId.ToString())).ToList();
            var audios = _context.DialogueAudios.Where(p => dialoguesIdName.Contains(p.DialogueId.ToString())).ToList();
            var speechs = _context.DialogueSpeechs.Where(p => dialoguesIdName.Contains(p.DialogueId.ToString())).ToList();
            var satisfactions = _context.DialogueClientSatisfactions.Where(p => dialoguesIdName.Contains(p.Dialogue.ToString())).ToList();
            foreach (var dialogue in dialogues)
            {
        ...
        public int TotalScoreInsideCalculate(IEnumerable<DialogueFrame> DF, DialogueAudio DA, double? PositiveTextTone)
        {
        ...
        public int BorderMoodCalculate(DialogueFrame dialogueFrame, DialogueInterval dialogueInterval)
        {
            var faceYawMax = _config.FaceYawMax;
            var faceYawMin = _config.FaceYawMin;
            var begScore = 0;
        ...
        public int BorderMoodCalculateList(List<DialogueFrame> dialogueFrame, List<DialogueInterval> dialogueInterval, int averageScore)
        {
            var score = 0;
            var lenght = 0;
        ...
        public void RewriteSatisfactionScore(string dialogueId)
        {
            ...
            try
            {
                satisfactionScore = _context.DialogueClientSatisfactions.First(p => p.DialogueId.ToString() == dialogueId);
            }
            catch
            {
