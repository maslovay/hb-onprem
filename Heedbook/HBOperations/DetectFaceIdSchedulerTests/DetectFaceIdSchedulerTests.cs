...
namespace DetectFaceIdScheduler.Tests
{
    [TestFixture]
    public class DetectFaceIdSchedulerTests : ServiceTest
    {
        private Process _detectFaceIdSchedulerProcess;
        ...
        private void RunServices()
        {
            try
            {
                ...
                {
                    _detectFaceIdSchedulerProcess = Process.Start(
                        "dotnet",
                        $"../../../../DetectFaceIdScheduler/bin/{config}/netcoreapp2.2/DetectFaceIdScheduler.dll --isCalledFromUnitTest true");
                }
                else
                {
                    _detectFaceIdSchedulerProcess = Process.Start(
                    "dotnet",
                    $"/app/HBOperations/DetectFaceIdScheduler/bin/{config}/netcoreapp2.2/DetectFaceIdScheduler.dll --isCalledFromUnitTest true");
                }
                Thread.Sleep(20000);
            }
            catch(Exception e)
            ...
            {                
                ...
                for(int i = 0; i < 50; i++)
                {
                    var Frame1 = new FileFrame()
                    {
                        ...
                        IsFacePresent = true,
                        FaceLength = 1,
                        FaceId = rnd.Next(0,2) == 1 || i == 0 ? faceId : null
                    };
                    var frameAttribute1 = new FrameAttribute()
                    {
                        ...
                        Gender = "Male",
                        Age = 20,
                        Value = "{\"Top\":"+rnd.Next(300, 400)+",\"Width\":"+rnd.Next(140, 180)+",\"Height\":"+rnd.Next(140, 180)+",\"Left\":"+rnd.Next(150, 180)+"}",
        ...
        [Test]
        public async Task CheckDetectFaceIdScheduler()
        {
            //Arrange
            ...
            try
            {
                ...
                var fileFrames = _repository.GetAsQueryable<FileFrame>().Where(p => p.DeviceId == _device.DeviceId
                        ...
                        && p.FaceId == null)
                    .ToList();
                System.Console.WriteLine($"fileFramesCount: {fileFrames.Count}");
                //Assert
        ...
        {
            await base.TearDown();    
            System.Console.WriteLine($"_detectFaceIdSchedulerProcess is null: {_detectFaceIdSchedulerProcess is null}");
            // _detectFaceIdSchedulerProcess.Kill();        
        }
