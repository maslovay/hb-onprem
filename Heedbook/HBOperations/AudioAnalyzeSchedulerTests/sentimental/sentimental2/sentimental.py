...
class Sentimental(object):
    def __init__(self, word_list=None, negation=None):
        if word_list is None and negation is None:
            ...
            word_list = [os.path.join(base_dir, p) for p in ['./word_list/russian.csv']]#['./word_list/afinn.csv', './word_list/russian.csv']]
    ...
    def load_word_list(self, filename):
        with open(filename, 'r', encoding="utf-8") as f:
            reader = csv.DictReader(f)
            word_list = {self.stemmer.stem(row['word']): float(row['score']) for row in reader}
    ...
    def analyze(self, sentence):
        ...
        if scores['positive'] - scores['negative'] > 0:
            positive_share = scores['positive']/(scores['positive'] - scores['negative'])
