...
namespace DialoguesRecalculateScheduler.Tests
{
    [TestFixture]
    public class DialoguesRecalculateSchedulerTests : ServiceTest
    {
        private Startup _startup;
        private Guid _dialogueClientProfileId;
        private Guid _TestDeviceId;
        private CompanyIndustry _industry;
        private Company _company;
        private Device _device;
        private Dialogue _dialogue;
        private DialogueClientProfile _dialogueClientProfile;
        private DialogueFrame _dialogueFrame;
        ...
        protected override void InitServices()
        {
            ...
            _dialogueRecalculateSetting = ServiceProvider.GetService<DialoguesRecalculateSchedulerSettings>();
        }
        ...
        protected override async Task PrepareTestData()
        {
            ...
            _repository.AddOrUpdate(_industry);
            ...
            _repository.AddOrUpdate(_company);
            ...
            _TestDeviceId = _device.DeviceId;
            _repository.AddOrUpdate(_device);        
        
            _dialogue = new Dialogue()
            {
                DialogueId = Guid.NewGuid(),
                CreationTime = DateTime.Now.AddHours(1),
                BegTime = DateTime.Now.AddMinutes(-15),
            ...
            _repository.AddOrUpdate(_dialogue);
            
            _dialogueClientProfileId = Guid.NewGuid();
            _dialogueClientProfile = new DialogueClientProfile()
            {
                DialogueClientProfileId = _dialogueClientProfileId,
                DialogueId = _dialogue.DialogueId,
                IsClient = true,
                Avatar= null,
                Age = 23,
                Gender = "female"
            };
            _repository.AddOrUpdate(_dialogueClientProfile);

            _dialogueAudio = new DialogueAudio()
            {
                DialogueAudioId = Guid.NewGuid(),
                DialogueId = _dialogue.DialogueId,
                IsClient = true,
                NeutralityTone = 0.2,
                PositiveTone = 0.5,
            ...
            _repository.AddOrUpdate(_dialogueAudio);

            _dialogueVisual = new DialogueVisual()
            {
                DialogueVisualId = Guid.NewGuid(),
                DialogueId = _dialogue.DialogueId,
                AttentionShare = 65.7,
                HappinessShare = 0.14450000040233135,
                NeutralShare = 0.36490000039339066,
                SurpriseShare = 0.0336000001989305,
                SadnessShare = 0.27499999925494195,
                AngerShare = 0.1820000022649765,
                DisgustShare = 0,
                ContemptShare = 0,
                FearShare = 0
            };
            _repository.AddOrUpdate(_dialogueVisual);
            
            _dialogueFrame = new DialogueFrame()
            {
                DialogueFrameId = Guid.NewGuid(),
                DialogueId = _dialogue.DialogueId,
                IsClient = true,
                Time = _dialogue.BegTime.AddMinutes(1),
                HappinessShare = 0.469,
                NeutralShare = 0.514,
                SurpriseShare = 0,
                SadnessShare = 0,
                AngerShare = 0,
            ...
            _repository.AddOrUpdate(_dialogueFrame);
            _repository.Save();
            ...
            var testDialogVideoCorrectFileName = _dialogue.DialogueId + ".mkv";
            var testDialogAudioCorrectFileName = _dialogue.DialogueId + ".wav";
        ...
        public async Task CheckSoundFilePresents()
        {
            ...
            //Act
            var testTask = _dialoguesRecalculateSchedulerJob.Execute(mockJobExecutionContext);
            await testTask;
            testTask.Wait();
            var _context = ScopeFactory.CreateScope().ServiceProvider.GetRequiredService<RecordsContext>();
            var dialogue = _context.Dialogues.FirstOrDefault(p => p.DialogueId == _dialogue.DialogueId);            

            //Assert
            Assert.IsFalse(dialogue is null);
            Assert.IsTrue(dialogue.StatusId == 6 || dialogue.StatusId == 3);   
            Assert.IsTrue(dialogue.Comment == "");         
        }
        [TearDown]
        ...
        {   
            try
            {
                _repository.Delete<DialogueClientProfile>(_dialogueClientProfile);
                _repository.Delete<Client>(p => p.CompanyId == _company.CompanyId);
                _repository.Delete<DialogueAudio>(p => p.DialogueAudioId == _dialogueAudio.DialogueAudioId);
                _repository.Delete<DialogueFrame>(p => p.DialogueFrameId == _dialogueFrame.DialogueFrameId);
                _repository.Delete<DialogueVisual>(p => p.DialogueVisualId == _dialogueVisual.DialogueVisualId);
                _repository.Delete<CompanyIndustry>(_industry);
                _repository.Delete<Company>(_company);
                _repository.Delete<Device>(_device);
                _repository.Delete<Dialogue>(_dialogue);
                _repository.Save();
            }
            catch(Exception e)
            {
