...
namespace PersonOnlineDetectionService.Tests
{
    [TestFixture]
    public class PersonOnlineDetectionServiceTests : ServiceTest
    {
        private Process _personOnlineDetectionServiceProcess;
        ...
        private void RunServices()
        {
            try
            {
                ...
                {
                    _personOnlineDetectionServiceProcess = Process.Start(
                        "dotnet",
                        $"../../../../PersonOnlineDetectionService/bin/{config}/netcoreapp2.2/PersonOnlineDetectionService.dll --isCalledFromUnitTest true");
                }
                else
                {
                    _personOnlineDetectionServiceProcess = Process.Start(
                        "dotnet",
                        $"/app/HBOperations/PersonOnlineDetectionService/bin/{config}/netcoreapp2.2/PersonOnlineDetectionService.dll --isCalledFromUnitTest true");
                }
                Thread.Sleep(10000);
            }
            catch(Exception e)
        ...
        protected override async Task PrepareTestData()
        {
            try
            {
                ...
                var testFramePath = Directory
                    .GetFiles(Path.Combine(currentDir, "Resources/Frames"), "testid_20190416085258.jpg")
                    .FirstOrDefault();

                if (testFramePath == null)
                    throw new Exception("Can't get a test Frame for preparing a testset!");

                testFrameId = Guid.NewGuid();
                System.Console.WriteLine($"TestFrameId: {testFrameId}");
                var testFrameCorrectFileName = testFrameId + ".jpg";
                var tasks = new List<Task>();
                if (!(await _sftpClient.IsFileExistsAsync("Frames/" + testFrameCorrectFileName)))
                {
                    System.Console.WriteLine($"upload: {testFrameCorrectFileName}");
                    await _sftpClient.UploadAsync(testFramePath, "clientavatars/", testFrameCorrectFileName);
                }
        ...
        [Test]
        public async Task PersonOnlineDetectionServiceTest()
        {
            //Arrange
             
                var model = new PersonOnlineDetectionRun()
                {
                    Path = $"clientavatars/{testFrameId}.jpg",
                    Attributes = $"{{\"Top\":309,\"Width\":137,\"Height\":170,\"Left\":709}}",
                ...
                Thread.Sleep(20000);
                client = _context.Clients.FirstOrDefault(p => p.CompanyId == company.CompanyId);
                System.Console.WriteLine($"client is null: {client is null} _clientId: {client.ClientId}");
                //Assert
        ...
        {
            await base.TearDown();    
            _personOnlineDetectionServiceProcess.Kill();        
        }
