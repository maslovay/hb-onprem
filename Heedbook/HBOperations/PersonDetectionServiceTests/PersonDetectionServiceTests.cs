...
namespace PersonDetectionService.Tests
{
    [TestFixture]
    public class PersonDetectionServiceTests : ServiceTest
    {
        private PersonDetection _personDetectionService;
        ...
        private DialogueClientProfile _dialogueClianfProfile;
        ...
        protected override async Task PrepareTestData()
        {
            ...
            _dialogueClianfProfile = new DialogueClientProfile()
            {
            ...
            _repository.AddOrUpdate<DialogueClientProfile>(_dialogueClianfProfile);
            _repository.Save();
        }
        protected override void InitServices()
        {
            _repository = ServiceProvider.GetService<IGenericRepository>();
            _personDetectionService = ServiceProvider.GetService<PersonDetection>();
        }
        ...
        {
            //Arrange
            var personDetectionRunModel = new PersonDetectionRun()
            {
                DeviceIds = new List<Guid>{_TestDeviceId}
            };

            //Act
            await _personDetectionService.Run(personDetectionRunModel);
            var companyId = _repository.GetAsQueryable<Device>().FirstOrDefault(p => p.DeviceId == _TestDeviceId).CompanyId;       
            
            //Assert
            Assert.IsFalse(_repository.GetAsQueryable<Client>().FirstOrDefault(p => p.CompanyId == companyId) is null);            
        }
        [TearDown]
        ...
        {   
            try
            {
                _repository.Delete<DialogueClientProfile>(_dialogueClianfProfile);
                _repository.Save();

                var checkDIalogueClientProfile = _repository.GetAsQueryable<DialogueClientProfile>()
                    .FirstOrDefault(p => p.DialogueClientProfileId == _dialogueClientProfileId);
                if( checkDIalogueClientProfile == null)
                    System.Console.WriteLine($"DialogueClientProfile {_dialogueClientProfileId} deleted");
                
                var companyId = _repository.GetAsQueryable<Device>().FirstOrDefault(p => p.DeviceId == TestDeviceId).CompanyId;       

                _repository.Delete<Client>(p => p.CompanyId == companyId);
                _repository.Save();
                var checkClient = _repository.GetAsQueryable<Client>().Where(p => p.CompanyId == companyId);
                if(checkDIalogueClientProfile is null)
