...
namespace HBLib.Utils {
    public class FFMpegWrapper {
        ...
        public double GetDuration(string fn) {
            ...
            string output = cmd.runCMD(ffPath, $"-i \"{fn}\"");

            string pattern = @"Duration: ([^,]+),";
            Regex rgx = new Regex(pattern);

            var m = Regex.Matches(output, pattern)[0];
            string captured = m.Groups[1].ToString();
        ...
        public string VideoToWav(string videoFn, string audioFn) {
            ...
            var cmd = new CMDWithOutput();
            return cmd.runCMD(this.ffPath, $@"-i {videoFn} -acodec pcm_s16le -ac 1 -ar 16000 -fflags +bitexact -flags:v +bitexact -flags:a +bitexact {audioFn}");
        }

        public List<Dictionary<string, string>> SplitBySeconds(string fn, double seconds, string directory = null, string rootFn = "split", bool convertToWebm = false) {
            fn = Path.GetFullPath(fn);
            var metadata = new List<Dictionary<string, string>>();
            double duration = GetDuration(fn);
            int NFiles = (int)Math.Ceiling(duration / seconds);

            string oldExtension = "." + fn.Split('.').Last();
            string newExtension = convertToWebm ? ".webm" : oldExtension;
            ...
            for (int i = 0; i < NFiles; i++) {
                ...
                double a = i * seconds;
                double b = (i + 1) * seconds;
                string newFn = $"{rootFn}_{i}{newExtension}";

                if (directory != null) {
                    newFn = Path.Combine(directory, newFn);
                }
                newFn = Path.GetFullPath(newFn);

                string arguments = $"-loglevel panic -i {fn} -ss {a} -to {b} -c copy";
        ...
        public string ConcatSameCodecs(List<string> fns, string outputFn, string dir = null) {
            ...
            foreach (var fn in fns) {
                content += $"file '{fn.Replace(Path.DirectorySeparatorChar.ToString(), @"/")}'\n";
            }
            
            File.WriteAllText(guidFn, content);
            string res = cmd.runCMD(this.ffPath, arguments);
        ...
        public string ConcatDifferentCodecs(List<string> fns, string outputFn) {
            ...
            return cmd.runCMD(this.ffPath, arguments);
        ...
        public string CutBlob(string fn, string outFn, string sTime, string eTime, bool outputSeek = false)
        {
            ...
            return cmd.runCMD(this.ffPath, args);
        }
